...
valid_hostname_suffixes = [string.strip() for string in open("domain_suffixes.txt")]
...
for root,dirs,files in os.walk(args.target_path):
    for path in files:
        ...
        for section in pe.sections:
            ...
            if sname not in sectionList:
                sectionList.append(sname)
        ...
        for sectionname in sectionList:
            network.add_node(sectionname,label=sectionname,color='blue',
...
hostname_network = bipartite.projected_graph(network, sectionnames)
